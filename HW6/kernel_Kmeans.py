...
def visualize(n, w, h, clusters, epoch, output_path):
    ...
    # img.save(os.path.join(output_path, "%03d.png" % epoch))

    return img

def Kernel_Kmeans(output_path, image, coords, w, h, gamma_s, gamma_c, K, mode, epoch):
    image_list = [] # list of images, for making gif.
    ...
    for e in range(1, epoch+1):
        ...
        img = visualize(n, w, h, final_clusters, e, output_path)
    ...
    # make gif
    image_list[0].save(os.path.join(output_path, "GIF.gif"), save_all = True, append_images = image_list[1:], duration = 200, loop = 0)
